...
def pixel_coordinate_transform(y, param=PARAMETERS):
        """
...
for vehicle_id_index, vehicle in enumerate(vehicle_pos_infos):
    ...
    for i in range(0, photo_numbers):
        if vehicle[i][2] - frame_1 >= DETECT_FRAME_SPAN - 1:
            ...
            real_pos_1 = pixel_coordinate_transform(vehicle[index_1][1])
            real_pos_2 = pixel_coordinate_transform(vehicle[index_2][1])
...
velocity_by_frame_json = json.dumps(velocity_by_frame)

with open(OUTPUT_PATH, mode='w') as velocity_file:
    velocity_file.write(velocity_by_frame_json)