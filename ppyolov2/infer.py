...
def main_video(
        ...
        video_save_path = "./ppvolo_res/video.mp4",
        sv_txt = "./ppvolo_res/det_detail.txt",
):
    FLAGS = parse_args()
...
def main_img(
        img_path,
        weights = "./models/19.pdparams",
        img_save_path = "./ppvolo_res/result.jpg",
):
    FLAGS = parse_args()
    # FLAGS包含先前parse_args中的所有参数，参数值要么为默认值，要么为命令行传入参数
    # 后面强制更改参数的值
...
# # 视频接口
# def reco_vedio(path, sv_txt=False):
#     FLAGS,cfg = main()
#     # 加载移动端预训练模型
#     ocr = hub.Module(name="chinese_ocr_db_crnn_server")
#     # build trainer
