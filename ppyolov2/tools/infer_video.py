...
# 检测车流视频
def run_video(FLAGS, cfg, video_path, video_save_path, sv_txt):
    # 加载移动端预训练模型
    ...
    if not os.path.exists('ppvolo_res'):
        os.mkdir('ppvolo_res') # 创建文件夹
    # 保存的视频地址
    output_viedo = cv2.VideoWriter(os.path.join('ppvolo_res',video_save_path), fourcc, fps, (width, height))
    while cap.isOpened():
        ...
        print("{}/{}".format(current_frame,frames_num),end=" ")# 可视化视频识别进度
        print(recog_res)
        if sv_txt:
            write_result(os.path.join('ppvolo_res',sv_txt), current_frame, bbox_lists, recog_res)
        current_frame += 1
...
def main_video(
    ...
    check_version()
    run_video(FLAGS,cfg,file_path,video_save_path,sv_txt)




if __name__ == '__main__':
    main_video("./car.mp4")  # 视频识别
